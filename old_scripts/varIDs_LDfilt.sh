...
vcftools --vcf ${vcfDir${sampleID}_mergeAP13.vcf.gz \
    --min-alleles 2 \
    --max-alleles 2 \
    --max-missing 0.01 \
    --minDP 10 \
    --minQ 29 \
    --remove-indels \
    --out ${sampleID}_filt_withAP13 \
    --recode 
    
# now using plink to set var ids 

plink2 --vcf ${sampleID}_filt_withAP13.recode.vcf --set-all-var-ids @:# --make-bed --out ${sampleID}_varIDs